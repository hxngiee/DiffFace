...
def show_editied_masked_image(
    ...
    targ_parser: Optional[Image] = None,
    src_parser:  Optional[Image] = None,
    path: Optional[Union[str, Path]] = None,
    distance: Optional[str] = None,
):
    fig_idx = 1
    rows = 1
    cols = 6 if mask is not None else 2
    # rows = 2
    ...
    if targ_parser is not None:
        fig.add_subplot(rows, cols, fig_idx)
        _set_image_plot_name("Target Parser")
        plt.imshow(targ_parser)
        fig_idx += 1

    if src_parser is not None:
        fig.add_subplot(rows, cols, fig_idx)
        _set_image_plot_name("X_in Parser")
        plt.imshow(src_parser)
        fig_idx += 1
